...
def save_content(req: SaveRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    item = SavedContent(user_id=current_user.id, type=req.type, title=req.title, content=req.content, session_id=req.session_id)
    db.add(item)
    db.commit()
...
def get_saved(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if not current_user:
        raise HTTPException(status_code=401, detail="Login required")
    items = db.query(SavedContent).filter(SavedContent.user_id == current_user.id).order_by(SavedContent.created_at.desc()).all()
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        if filename.endswith(".pdf"):
            ...
            response = client.chat.completions.create(
                ...
                messages=[{"role":"user","content":[{"type":"image_url","image_url":{"url":f"data:{mime};base64,{b64}"}},{"type":"text","text":"Extract and transcribe all text from this image accurately."}]}],
                max_tokens=2000
            )
    ...
    return {"session_id": session_id, "message": "Uploaded successfully", "preview": text[:300], "word_count": len(text.split())}
...
async def chat(req: ChatRequest):
    ...
    return {"answer": ask(f"{LANGUAGE_INSTRUCTION}\nYou are a helpful study assistant. Answer questions based on these notes:\n\n{notes}\n\nDetect the user's language and respond in that same language.", req.question)}

@app.post("/summarize")
async def summarize(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"summary": "No notes found."}
    return {"summary": ask(f"{LANGUAGE_INSTRUCTION}\nSummarize these notes with key points and main ideas. Use the same language as the notes.", notes)}

@app.post("/quiz")
async def quiz(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"quiz": "No notes found."}
    return {"quiz": ask(f"{LANGUAGE_INSTRUCTION}\nGenerate exactly {req.count} multiple choice questions.\nFormat:\nQ: question\nA) option\nB) option\nC) option\nD) option\nAnswer: X\n\nSeparate with blank lines. Use same language as notes.", notes)}

@app.post("/flashcards")
async def flashcards(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"flashcards": "No notes found."}
    return {"flashcards": ask(f"{LANGUAGE_INSTRUCTION}\nCreate exactly {req.count} flashcards.\nFormat:\nFront: question\nBack: answer\n---\nUse same language as notes.", notes)}

@app.post("/exam_predictor")
async def exam_predictor(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"exam_predictor": "No notes found."}
    return {"exam_predictor": ask(f"{LANGUAGE_INSTRUCTION}\nYou are an expert exam predictor. Based on these notes, predict the {req.count} most likely exam questions. For each question explain WHY it's likely to appear. Use same language as notes.", notes)}

@app.post("/study_plan")
async def study_plan(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"study_plan": "No notes found."}
    return {"study_plan": ask(f"{LANGUAGE_INSTRUCTION}\nCreate a detailed 7-day study plan based on these notes. For each day specify: topics to cover, activities, time needed, and goals. Use same language as notes.", notes)}

@app.post("/key_terms")
async def key_terms(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"key_terms": "No notes found."}
    return {"key_terms": ask(f"{LANGUAGE_INSTRUCTION}\nExtract the {req.count} most important key terms and concepts from these notes. For each term provide: the term, its definition, and why it's important. Use same language as notes.", notes)}

@app.post("/mind_map")
async def mind_map(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"mind_map": "No notes found."}
    return {"mind_map": ask(f"{LANGUAGE_INSTRUCTION}\nCreate a detailed mind map outline.\nFormat:\nMain Topic\n  - Subtopic\n    * Detail\n    * Detail\n  - Subtopic\n    * Detail\nUse same language as notes.", notes)}
...
async def eli5(req: Eli5Request):
    ...
    return {"eli5": ask(f"{LANGUAGE_INSTRUCTION}\nExplain '{topic}' as simply as possible, like explaining to a 5 year old. Use simple words, fun analogies, and real life examples. Detect language from the topic and respond in same language.", f"{context}Explain: {topic}")}

@app.post("/formula_sheet")
async def formula_sheet(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"formula_sheet": "No notes found."}
    return {"formula_sheet": ask(f"{LANGUAGE_INSTRUCTION}\nExtract all formulas, equations, and mathematical expressions from these notes. Format as a clean reference sheet with: formula name, the formula itself, variables explained, and when to use it. Use same language as notes.", notes)}

@app.post("/chapter_summary")
async def chapter_summary(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"chapter_summary": "No notes found."}
    return {"chapter_summary": ask(f"{LANGUAGE_INSTRUCTION}\nDivide these notes into chapters or sections and summarize each one separately. Use clear headings for each section. Use same language as notes.", notes)}

@app.post("/simplify_words")
async def simplify_words(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"simplify_words": "No notes found."}
    return {"simplify_words": ask(f"{LANGUAGE_INSTRUCTION}\nRewrite these notes in the simplest possible language. Replace difficult words with easy ones. Keep all the information but make it easy to understand. Use same language as notes.", notes)}

@app.post("/fill_blanks")
async def fill_blanks(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"fill_blanks": "No notes found."}
    return {"fill_blanks": ask(f"{LANGUAGE_INSTRUCTION}\nCreate exactly {req.count} fill-in-the-blank exercises from these notes.\nFormat:\n1. Sentence with _______ blank\nAnswer: missing word\n\nUse same language as notes.", notes)}

@app.post("/true_false")
async def true_false(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"true_false": "No notes found."}
    return {"true_false": ask(f"{LANGUAGE_INSTRUCTION}\nCreate exactly {req.count} true/false questions from these notes.\nFormat:\n1. Statement\nAnswer: True/False\nExplanation: why\n\nMix true and false equally. Use same language as notes.", notes)}

@app.post("/short_answer")
async def short_answer(req: ActionRequest):
    notes = get_notes(req.session_id)
    if not notes: return {"short_answer": "No notes found."}
    return {"short_answer": ask(f"{LANGUAGE_INSTRUCTION}\nCreate exactly {req.count} short answer questions from these notes.\nFormat:\nQ: question\nA: answer (2-3 sentences)\n\nUse same language as notes.", notes)}
...
async def debate(req: DebateRequest):
    ...
    context = f"Using these notes as context:\n{notes}\n\n" if notes else ""
    return {"debate": ask(f"{LANGUAGE_INSTRUCTION}\nPresent a balanced debate on '{topic}'.\nFormat:\n## Arguments FOR\n- point 1\n- point 2\n\n## Arguments AGAINST\n- point 1\n- point 2\n\n## Conclusion\nBalanced summary\n\nDetect language from topic and respond in same language.", f"{context}Debate topic: {topic}")}

@app.post("/essay_grade")
async def essay_grade(req: EssayRequest):
    notes = get_notes(req.session_id)
    context = f"Reference notes:\n{notes}\n\n" if notes else ""
    return {"essay_grade": ask(f"{LANGUAGE_INSTRUCTION}\nYou are an expert essay grader. Grade this essay and provide detailed feedback.\nFormat:\n## Overall Grade: X/10\n\n## Strengths\n- point\n\n## Areas to Improve\n- point\n\n## Detailed Feedback\nParagraph by paragraph analysis\n\n## Suggestions\nSpecific improvements\n\nDetect language from essay and respond in same language.", f"{context}Essay to grade:\n{req.essay_text}")}

@app.post("/homework_help")
async def homework_help(req: HomeworkRequest):
    notes = get_notes(req.session_id)
    context = f"Reference notes:\n{notes}\n\n" if notes else ""
    return {"homework_help": ask(f"{LANGUAGE_INSTRUCTION}\nYou are an expert {req.subject} tutor. Help solve this problem step by step.\nFormat:\n## Understanding the Problem\n\n## Step-by-Step Solution\nStep 1: ...\nStep 2: ...\n\n## Answer\n\n## Key Concepts Used\n\nDetect language from question and respond in same language.", f"{context}Subject: {req.subject}\nQuestion: {req.question}")}
...
async def compare(req: CompareRequest):
    ...
    return {"compare": ask(f"{LANGUAGE_INSTRUCTION}\nCompare these two documents thoroughly.\nFormat:\n## Document 1 Summary\n\n## Document 2 Summary\n\n## Key Similarities\n- point\n\n## Key Differences\n- point\n\n## Which is More Comprehensive?\n\n## Conclusion\n\nUse English for comparison.", f"Document 1:\n{notes1[:3000]}\n\nDocument 2:\n{notes2[:3000]}")}
